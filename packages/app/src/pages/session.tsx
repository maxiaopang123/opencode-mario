...
export default function Page() {
  ...
  const desktopFilePreviewOpen = createMemo(() => isDesktop() && layout.filePreview.opened() && !!layout.filePreview.path())
  const desktopSidePanelOpen = createMemo(
    () => desktopSessionResizeOpen() || desktopFileTreeOpen() || desktopFilePreviewOpen(),
  )
  const sessionPanelWidth = createMemo(() => {
    if (!desktopSidePanelOpen()) return "100%"
    if (desktopSessionResizeOpen()) return `${layout.session.width()}px`
    const width = [
      desktopFilePreviewOpen() ? layout.filePreview.width() : 0,
  ...
  createEffect(
    on(
      desktopReviewOpen,
      (reviewOpen) => {
        if (reviewOpen && layout.filePreview.opened()) layout.filePreview.close()
      },
      { defer: true },
    ),
  )
  ...
  const openSessionPreview = (target: string) => {
            ...
            <Show when={messagesReady() ? params.id : undefined} keyed>
              {(_id) => (
                <DataProvider
                  ...
                  onSessionHref={hrefToSession}
                  onOpenPreview={openSessionPreview}
                >
                  <MessageTimeline
                    actions={actions}
                    scroll={ui.scroll}
  ...
  return (
    <SessionRouteFrame>
      ...
      >
        ...
        <Show when={desktopFilePreviewOpen()}>
          <div
            class="relative h-full shrink-0 overflow-hidden border-l border-border-weaker-base bg-background-base"
            style={{ width: `${layout.filePreview.width()}px` }}
          >
            <FilePreviewPanel path={layout.filePreview.path()!} />
            <div onPointerDown={() => size.start()}>
              <ResizeHandle
                direction="horizontal"
                edge="start"
                size={layout.filePreview.width()}
                min={320}
                max={800}
                onResize={(width) => {
                  size.touch()
                  layout.filePreview.resize(width)
                }}
              />
            </div>
          </div>
