...
async function highlightCodeBlocks(html: string): Promise<string> {
  ...
  for (const match of matches) {
    ...
    let language = normalizeCodeLanguage(lang)
    if (language === "mermaid") {
      result = result.replace(fullMatch, () => specialCodePlaceholder("mermaid", code))
      continue
    }
    if (language === "html") {
      result = result.replace(fullMatch, () => specialCodePlaceholder("html", code))
      continue
    }
...
function specialCodePlaceholder(language: "mermaid" | "html", code: string) {
  return `<div data-${language}-block="${encodeURIComponent(code)}"></div>`
}
    ...
    const jsParser = marked.use(
      ...
      markedShiki({
        async highlight(code, lang) {
          const specialLanguage = normalizeCodeLanguage(lang)
          if (specialLanguage === "mermaid") return specialCodePlaceholder("mermaid", code)
          if (specialLanguage === "html") return specialCodePlaceholder("html", code)
          ...
          const language = specialLanguage in bundledLanguages ? specialLanguage : "text"
          if (!highlighter.getLoadedLanguages().includes(language)) {
            await highlighter.loadLanguage(language as BundledLanguage)
          }
      ...
      }),
      {
        renderer: {
          code({ text, lang }) {
            ...
            if (language === "mermaid") return specialCodePlaceholder("mermaid", text)
            if (language === "html") return specialCodePlaceholder("html", text)
            return false
          },
        },
      },
